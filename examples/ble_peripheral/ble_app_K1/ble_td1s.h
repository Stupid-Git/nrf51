...
#define BLE_UUID_TD1S_SERVICE 0x0001                      /**< The UUID of the Nordic UART Service. */
#define BLE_TD1S_MAX_DATA_LEN (GATT_MTU_SIZE_DEFAULT - 3) /**< Maximum length of data (in bytes) that can be transmitted to the peer by the Nordic UART service module. */

/* Forward declaration of the ble_td1s_t type. */
typedef struct ble_td1s_s ble_td1s_t;

/**@brief Nordic UART Service event handler type. */
typedef void (*ble_td1s_data_handler_t) (ble_td1s_t * p_td1s, uint8_t * p_data, uint16_t length);

/**@brief Nordic UART Service initialization structure.
 *
 * @details This structure contains the initialization information for the service. The application
 * must fill this structure and pass it to the service using the @ref ble_td1s_init
 *          function.
 */
typedef struct
{
    ble_td1s_data_handler_t data_handler; /**< Event handler to be called for handling received data. */
} ble_td1s_init_t;
 ...
 *
 * @param[out] p_td1s      Nordic UART Service structure. This structure must be supplied
 *                        by the application. It is initialized by this function and will
 *                        later be used to identify this particular service instance.
 * @param[in] p_td1s_init  Information needed to initialize the service.
 *
 * @retval NRF_SUCCESS If the service was successfully initialized. Otherwise, an error code is returned.
 * @retval NRF_ERROR_NULL If either of the pointers p_td1s or p_td1s_init is NULL.
 */
 ...
 *          peer.
 *
 * @param[in] p_td1s       Pointer to the Nordic UART Service structure.
